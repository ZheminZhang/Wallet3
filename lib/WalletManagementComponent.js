...
class ManagementList extends Component<Props> {
  constructor(props) {
    super(props);
    setTimeout(() => this.getCoinPrice(), 1000)
    this.state = {
      walletLength: 0,
      wallet: {},
      coinPrice: 0
  ...
  getCoinPrice(){
    ...
    ws.onmessage = (e) => {
      ...
      response.RMBRate = JSON.parse(e.data).RMBRate;
      this.setState({ coinPrice: response.ETHPrice*response.RMBRate/100 });
    }
  }
      ...
      <ScrollView style={{ backgroundColor: "white" }}>
        <View style={styles.Background}>
          <View style={{marginTop: 20, marginBottom: 10}}>
            <Text style={{color: "#272727", fontSize: 26, textAlign: "center", fontFamily: "Chalkduster"}}>1 ETH = {this.state.coinPrice.toFixed(2)} RMB</Text>
          </View>
