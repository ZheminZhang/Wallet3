...
class ContractHomeList extends Component<Props> {
  ...
  A() {
    httpRequest({ loadContract: [state1] }, response => {
      if (response.err) {
        alert(response.err);
      }
        ...
        >
          <ScrollView style={{ height: 0.6 * height, width: 0.5 * width }}>
            <ContractList
              ...
              contractState={this.props.contractState1}
            />
          </ScrollView>
...
const MyContract = StackNavigator({
  ...
  Contract2: { screen: ContractInterface }
});
...
export default class LoadSmartContract extends React.Component {
  constructor(props) {
    ...
    state1 = this.props.contractState1;
  }
  render() {
    return <MyContract />;
  }
