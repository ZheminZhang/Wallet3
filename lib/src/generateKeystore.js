  ...
  return new Promise(function(resolve, reject) {
    ...
    keystore.deriveKeyFromPasswordAndSalt(password, salt, async function(err, pwDerivedKey) {
        ...
        if (!privKey) {
          ...
          realm.write(()=> {
            realm.create('Wallet', {address: ks.addresses[0], name: walletName, type: walletType, keystore: ks_string, isSeed: "true"});
          })
        } else {
          importPrivKey(ks, privKey, pwDerivedKey, walletType);
          var ks_string = ks.serialize();
          realm.write(()=> {
            realm.create('Wallet', {address: ks.addresses[0], name: walletName, type: walletType, keystore: ks_string, isSeed: "false"});
          })
        }
        // var ks_string = ks.serialize();
        // realm.write(()=> {
