...
export default class DeploySmartContract extends Component<Props> {
  ...
  onButtonPress(index) {
    //callSmartContract(this.state.password, txOptions, global.ws)
    txOptions = {};
    ...
    if (JSON.parse(this.props.callConfig)[index].constant) {
      inquireSmartContract(this.props.ws, this.props.abi, JSON.parse(this.props.callConfig)[index].name, this.state.args[index], this.props.contractAddr)
    }
    else {
