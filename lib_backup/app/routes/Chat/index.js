...
export default class Chat extends Component {
  static navigationOptions = ({ navigation }) => ({
    title: navigation.state.params.conversation.username
  });
  ...
  convertJMessageToAuroraMsg(jmessage) {
    ...
    auroraMsg.timeString = jmessage.createTime;
  ...
  sendCustomMessage = () => {
    ...
    for (var i = 0; i < 10; i++) {
      ...
      message.content =
        '<body bgcolor="#ff3399"><h5>This is a custom message. </h5>\
    ...
    this.messageListDidLoadCallback = () => {
      JMessage.getHistoryMessages(
        ...
        messages => {
          ...
          //var time = this.convertJMessageToAuroraMsg(messages[0]);
          ...
          //alert(JSON.stringify(auroraMessages))
  ...
  onSendGalleryFiles = mediaFiles => {
    for (index in mediaFiles) {
      Alert.alert(
        "onSendGalleryFiles",
        JSON.stringify(mediaFiles[index]["mediaPath"])
      );
      var message = this.getNormalMessage();
      message.messageType = "image";
    ...
    return (
      <View style={styles.container}>
        <MessageListView
          ...
          receiveBubblePadding={{ left: 15, top: 10, right: 10, bottom: 10 }}
        />
        <InputView
